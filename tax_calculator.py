...
def cli():
    """Non-interactive CLI mode. Called when --input flag is present."""
    import argparse, sys as _sys
    parser = argparse.ArgumentParser(description="Irish Tax Calculator CLI")
    ...
    # Build dataclasses from dicts, ignoring unknown keys
    import dataclasses
    profile_fields     = {f.name for f in dataclasses.fields(UserProfile)}
    investments_fields = {f.name for f in dataclasses.fields(Investments)}

    profile     = UserProfile(**{k: v for k, v in profile_data.items()     if k in profile_fields})
    investments = Investments(**{k: v for k, v in investments_data.items() if k in investments_fields})

    if mode == "optimize":
        required_liquid = float(data.get("required_liquid_cash", 0.0))
        optimal = IrishTaxCalculator.optimize(profile, investments, required_liquid)
        calc_result = IrishTaxCalculator.calculate(profile, optimal)
        output = {
            "mode": "optimize",
            "investments": {
                "pension_contribution":      round(optimal.pension_contribution, 2),
                "cycle_to_work":             round(optimal.cycle_to_work, 2),
                "travel_pass":               round(optimal.travel_pass, 2),
                "income_protection_premium": round(optimal.income_protection_premium, 2),
                "voucher_allocation":        round(optimal.voucher_allocation, 2),
            },
            "limits": {
                "max_pension":     round(IrishTaxCalculator.get_max_pension_limit(profile), 2),
                "max_cycle":       round(IrishTaxCalculator.get_max_cycle_to_work_limit(investments), 2),
                "max_travel":      1830.0,
                "max_ip":          round(profile.gross_income * 0.10, 2),
            },
            "calculation": calc_result,
        }
    else:
        result = IrishTaxCalculator.calculate(profile, investments)
        output = {"mode": "calculate", "calculation": result}
...
if __name__ == "__main__":
    ...
    if "--input" in _sys_main.argv:
        cli()
    else:
        pass  # fall through to interactive block below


if __name__ == "__main__" and "--input" not in __import__("sys").argv:
